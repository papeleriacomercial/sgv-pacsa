...
| 3a | Registro en la bitácora de la cuenta y próximo paso a 3 días | **Construida** — 2 de septiembre de 2026 |
| 3b | Catálogo de marcas de competencia con sus metrajes medidos | Pendiente |
...
| | Dónde va |
|---|---|
| El catálogo de marcas de competencia y sus metrajes medidos | **Etapa 3b** |
| Normalizar el nombre de una marca escrita distinto por dos vendedores | **Etapa 3b** |
...
| Tabla `comparaciones` y su depósito | **Falta correr `npx supabase db push`** | **No** |
...
# ETAPA 3a — Queda en la bitácora, y deja un próximo paso

## Por qué, con las palabras del usuario

*«El vendedor va a estar haciendo varias de estas comparaciones en diferentes clientes o prospectos,
y no va a tener la memoria de qué fue lo que le ofreció a uno en particular.»* — 2 de septiembre de
2026.

**Es un argumento más fuerte que el del documento de requerimientos.** Ahí decía «guarda el cálculo
en la ficha del cliente»; acá está el porqué: **la hoja se la lleva el cliente y nosotros nos
quedamos sin copia**. El vendedor vuelve a los tres días a un local donde ofreció un precio escrito a
mano, y no tiene cómo saber cuál fue.

## Lo que se guarda

Tabla `comparaciones`, una fila por hoja generada:

| | |
|---|---|
| Cuándo, quién y a qué cuenta | El hecho |
| La marca que el cliente usa hoy | Vuelve el campo, como texto libre hasta que exista el catálogo |
| **Los cinco datos del cliente** | Los que quiso dar; los demás nulos |
| **Los cuatro nuestros**, calibre incluido | El precio que se escribió a mano ese día |
| **El resultado que se le enseñó** | Costo por metro de cada uno, cajas equivalentes, ahorro por pedido y diferencia al año |

**El resultado se guarda aunque se pueda recalcular**, y es deliberado: es el registro de **lo que se
le dijo al cliente**, no una vista derivada. El día que se toque una fórmula, lo que quedó dicho
aquella tarde no puede cambiar retroactivamente.

## La ruta del depósito no admite tildes

**«Invalid key».** Lo encontró el usuario con la primera comparación real, el 2 de septiembre de
2026. La ruta se armaba con el nombre bonito del archivo —«Comparación de costo — Abarrotería
Jessie.xlsx»— y **el depósito sólo acepta caracteres simples**: una tilde o una raya larga la
rechazan. La cotización nunca lo topó porque su archivo se llama `COT-0001.pdf`.

Adentro se guarda como `<identificador>/comparacion.xlsx`. **La carpeta es el identificador**, que
es lo que el permiso comprueba; el nombre de adentro no tiene que distinguir nada. Al bajarlo desde
la ficha se le devuelve su nombre con el del cliente, porque si no caerían archivos idénticos en la
carpeta de descargas.

Hay una prueba que lo cuida, y **comprueba las dos mitades**: que la ruta sólo lleve caracteres
admitidos, y que el nombre bonito —que sí lleva tildes, y así debe ser— nunca acabe siendo parte de
ella.

## Volver al expediente tiene que tirar la copia vieja

**El defecto lo encontró el usuario y también dio con la causa.** Grabó la comparación, salió el
aviso verde, subió con el dedo, tocó «volver»… y la sección no estaba. Al recargar a mano, sí.

*«Al volver, tuvo que haber refrescado la pantalla, pero no lo hizo.»*

**El botón de volver no recarga: navega.** Y la aplicación guarda una copia de cada pantalla ya
visitada, así que devolvía el expediente **tal como estaba antes de la comparación**. La bitácora de
la visita sí aparecía porque ya existía cuando esa copia se guardó — que es justo lo que hacía creer
que la comparación no se había grabado.

Ahora, al terminar, aparece **«Volver al expediente»**, que tira la copia guardada y recién entonces
navega. Es lo que propuso el usuario.

### Y no se recarga la página entera, a propósito

La salida obvia —forzar una recarga completa— **está descartada por algo que el usuario reportó en la
misma frase**: *«si yo arrastro el dedo hacia abajo para refrescar, me tira a volver a hacer login.
Estoy en un iPhone»*.

**Una recarga completa le haría lo mismo**, y perdería el trabajo. Se navega con el enrutador, que
no vuelve a arrancar la aplicación.

> **Pendiente, y es de otro módulo:** que un refresco devuelva a la pantalla de ingreso en el iPhone
> es grave para el uso en la calle, y no se pudo diagnosticar desde acá — el proxy que mantiene la
> sesión está correctamente escrito. **Hace falta reproducirlo con el aparato delante.**

## Una comparación se puede borrar, y se lleva su seguimiento

**No había forma de borrar una**, y el usuario la necesitó el mismo día que se construyó: la primera
comparación real quedó sin su archivo por el defecto de la ruta, sobre un prospecto que ya tenía
información. Un registro equivocado en la ficha de un cliente real no se puede dejar ahí.

**Se marca como borrada, no se borra de verdad** — como el resto del sistema, y acá importa más que
en otros lados: esto es el registro de lo que se le dijo a un cliente.

**Y se va con su seguimiento.** El compromiso que dejó dice, con todas las letras, «dar seguimiento a
la comparación de costo entregada a X»: si la comparación desaparece y él se queda, **en la agenda
del vendedor sobrevive una tarea que apunta a algo que ya no existe**.

Va **al final de la tarjeta y en letra pequeña**, con confirmación, y **sólo para el dueño de la
cuenta**: a otro los permisos se lo rebotarían, y ofrecer un botón que va a fallar es peor que no
ofrecerlo.

## Generar no confirmaba nada

**Se compartía el archivo y la pantalla quedaba igual que antes.** El mismo día, el usuario probó
contra una compilación vieja que todavía no grababa y **la pantalla no le dio ninguna señal**: tuvo
que deducirlo. «Grabó» y «no grabó» se veían idénticos.

Ahora se dice, y en tres tonos distintos, porque son tres cosas distintas:

| | |
|---|---|
| **Quedó en la ficha** | Con la copia del archivo, y con el día en que toca volver |
| **Guardado en este teléfono** | Sin señal: entra a la ficha en cuanto haya. **No es una falla**, y decirlo como si lo fuera haría que el vendedor repita la comparación y queden dos |
| **No se guardó** | Con el motivo |

## La copia del archivo, tal cual

**Se sube el mismo `.xlsx` que recibió el cliente**, a un depósito privado, por el mismo camino que
la cotización sube su PDF — **con cola para cuando no hay señal**. Decisión del usuario: al volver al
local se abre el archivo que él tiene, no uno parecido.

Rehacerlo a partir de los números habría salido más barato, pero **la hoja rehecha usaría la
plantilla de hoy**, y la plantilla cambió cuatro veces en un solo día. Un documento que no es el que
el cliente tiene sirve para poco en una conversación sobre lo que se prometió.

El depósito es **privado**, por lo mismo que el de cotizaciones: la hoja lleva un precio hecho para
un cliente concreto.

## El próximo paso a tres días

Se crea **solo**, en la tabla `compromisos` — *«Próximo paso con fecha. Es el motor del seguimiento
diario»*—, así que **cae en la agenda que el vendedor ya mira cada mañana**. No hubo que inventar
nada: es la misma pieza que usa «Programar seguimiento», con `visita_id` en nulo porque no nace de
una visita registrada.

**La fecha se muestra antes de generar y se puede cambiar ahí mismo.** Es lo que pide el documento
—*«con fecha editable por el vendedor»*— y tiene una razón de piso: el vendedor sabe si ese local no
abre el jueves, y corregirlo después desde la agenda es un paso que nadie da.

**Tres días no es un número redondo elegido al azar:** es la única señal disponible. No hay forma de
saber si el cliente abrió la hoja.

## Lo que no detiene la entrega

El orden importa. **Primero se arma el archivo y se comparte**; el registro, la subida y el
compromiso van después. Si algo de eso falla —sin señal, que en la calle es lo normal— **el cliente
igual se lleva su hoja**, y lo pendiente espera en la cola.

**Lo que no se encola es el archivo grande.** La cola lo descarta por encima de su tope, con el mismo
criterio que la cotización: se prefiere perder la copia antes que el registro.
...
| Fecha | Qué cambió |
|---|---|
| 2026-09-02 | **Etapa 3a construida**: cada comparación queda en la bitácora de la cuenta con sus datos, su resultado y una copia del archivo, y deja un próximo paso a 3 días de fecha editable. *«El vendedor no va a tener la memoria de qué fue lo que le ofreció a uno en particular»* |
